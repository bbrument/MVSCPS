...
# Submit LUCES-MV training (native loader) with POINT light for all 10 objects
...
echo ""
echo "================================================"
echo "Submitting LUCES-MV Native POINT Light Training"
echo "  Depends on preprocessing job: ${PREPROC_JOB_ID}"
echo "  Data: /projects/m25115/LucesMV_processed"
echo "  Exp:  /projects/m25115/exp/lucesmv_native_point"
echo "  Eval: lucesmv/eval/*/mvscps-native-point/nbv-12/nbl-15/nbit-20000"
echo "  Mesh: mc1024"
echo "================================================"
echo ""
...
for obj in Bowl Buddha Bunny Cup Die Hippo House Owl Queen Squirrel; do
  JOB_ID=$(sbatch --parsable \
    ...
    --job-name="lmv-npt-${obj}" \
    --output="slurm/logs/lmv-npt-${obj}_%j.log" \
    --error="slurm/logs/lmv-npt-${obj}_%j.err" \
    --export=ALL,LUCESMV_LIGHT_TYPE=point \
    slurm/run_lucesmv_native.sh "$obj" "$@")
  echo "Submitted ${obj} (native-point): Job ID ${JOB_ID}"
done

echo ""
echo "================================================"
echo "All 10 LUCES-MV native point light jobs submitted."
