...
def safe_crop_image(img, mask):
    cropped = crop_image_by_mask(img, mask, 0)
    return cropped if cropped is not None else np.zeros_like(img)
...
def crop_image_by_mask(img, mask, boarder=0, fill_background=True):
    if fill_background:
        if img.max() > 1.:
            img[~mask] = 255
        else:
    ...
    try:
        ...
        crop_img = img.copy()[bbox[1]-boarder:bbox[3]+boarder, bbox[0]-boarder:bbox[2]+boarder]
    except:
        crop_img = img.copy()
    return crop_img


def apply_jet_on_error_maps(err_maps, cb_up=1):
    err_jets = []
    for err_map in err_maps:
        err_map = err_map / cb_up  # 10mm
        err_map[err_map > 1] = 1
...
def get_parameters(model, name):
    ...
    if isinstance(module, nn.Module):
        ...
        # print([isinstance(m, nn.Module) for m in module])
        return [m.parameters() for m in module]
    return []
